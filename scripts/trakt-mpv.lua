...
local client_id = mysplit(lines[5], ":")
...
local function checkin()
    local status, output = evoque_python({"--query", mp.get_property('media-title'), "0.00", "yes"})
...
local function on_file_start(event)
    ...
    elseif status == 0 then
        ...
        while( info ~= 26 )
        do
            local info, scrobble = evoque_python({"--query", mp.get_property('media-title'), mp.get_property('percent-pos'), mp.get_property('pause')})
            if info == 26 then
                send_message("Scrobble:" .. scrobble, "00FF00", 2)
                --print("break")
                return
            elseif info == -2 then
                --file = io.open(config, "rb")
